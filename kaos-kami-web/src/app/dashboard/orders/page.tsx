  ...
  return (
    <div className="min-h-screen bg-canvas text-text-primary flex flex-col justify-between">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 py-10 sm:py-16 space-y-10">
        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 pb-6 border-b border-border-subtle">
          ...
          <Link
            href="/studio"
            className="py-2.5 px-4 rounded-xl bg-brand-accent text-canvas font-mono font-bold text-xs uppercase tracking-wider hover:brightness-110 active:scale-95 transition-all dark:shadow-[0_0_16px_rgba(230,81,0,0.3)] w-fit"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="bg-surface border border-border-subtle rounded-2xl overflow-hidden divide-y divide-border-subtle font-mono text-xs dark:shadow-xl">
            {recentOrders.length > 0 ? (
              recentOrders.map((order) => (
                <div
                  key={order.id}
                  className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-surface-elevated/40 transition-colors"
                >
                  ...
                  <div className="flex flex-wrap items-center justify-between sm:justify-end gap-2 sm:gap-x-4">
                    ...
                    <Link
                      ...
                      className="px-3.5 py-1.5 rounded-xl bg-surface border border-border-subtle hover:border-brand-accent text-text-primary font-bold hover:text-brand-accent transition-all flex items-center gap-1 text-[11px]"
                    >
                      <span>INVOICE</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {savedDesigns.length > 0 ? (
              savedDesigns.map((design) => (
                <div
                  key={design.id}
                  className="p-4 rounded-2xl bg-surface border border-border-subtle space-y-3 font-mono text-xs hover:border-border-strong transition-all dark:shadow-md"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <div
                  ...
                  <div className="p-3 rounded-xl bg-surface/50 border border-border-subtle flex items-center justify-between text-[11px]">
                  ...
                  <Link
                    ...
                    className="w-full py-2 rounded-xl bg-surface border border-border-subtle hover:border-brand-accent text-text-primary font-bold text-center block transition-all hover:text-brand-accent text-[11px]"
                  >
                    BUKA DI 3D STUDIO
                  </Link>
              ...
              <div className="col-span-full p-8 text-center text-text-muted bg-surface border border-border-subtle rounded-2xl">
