...
export default async function RenderPage({ params }: { params: Promise<{ designId: string }> }) {
  const { designId } = await params;
  const design = await db.query.Design.findFirst({ where: (t, { eq }) => eq(t.id, designId), with: { category: true } });
