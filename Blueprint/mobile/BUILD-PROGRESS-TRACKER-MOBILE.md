...
| **14 Sep 2026** | **Tema Dark/Light** | Unifikasi token web (`primary #E65100`) + extend Tailwind `canvas/surface/text/border/success`; `TabBar/BottomSheet/GlassCard/NativeHeader/CanvasStageMobile` → token; `MobileStudioLighting` prop `theme`; iOS Dark + storyboard `#0E0E10`; Android `windowBackground #0E0E10`; hapus double `pt-safe` | `npx tsc --noEmit` mobile 0 error, tanpa build APK (GATE) | 🟢 Selesai (kode) |
| **14 Sep 2026** | **All-Device + Draco** | Soft-disable Draco → non-Draco cermin web + arsip `backups/draco-archive/`; sheet `88dvh`/snap 50%/`repositionInputs`; `100dvh` + safe-area; backButton; push tap; retry + order tamu; downscale 1024; gizmo/haptics/share fallback | `npx tsc --noEmit` mobile 0 error (verifikasi ulang), tanpa build APK (GATE) | 🟢 Selesai (kode) |
