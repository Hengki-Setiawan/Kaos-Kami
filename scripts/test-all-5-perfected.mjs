...
function getApparelSleevePlacement(apparel, targetSide, decalX, decalY) {
  const isRight = targetSide === "right_sleeve";
  const sign = isRight ? -1 : 1;

  // decalY in [-0.35, +0.35]
  // progress = 0 (top/shoulder) to 1 (bottom/cuff)
  ...
  if (apparel === "longsleeve") {
    ...
    y = 0.22 - progress * 0.12; // +0.22 to +0.10
    ...
    outerX = -0.28 - 0.32 * Math.sin(progress * Math.PI * 0.5);
    z = -0.025 + progress * 0.065; // -0.025 to +0.040
    normX = -0.80;
    normY = 0.58;
    depth = 0.070;
  } else if (apparel === "hoodie") {
    y = 0.10 - progress * 0.35; // +0.10 to -0.25
    outerX = -0.19 - 0.245 * Math.sin(progress * Math.PI * 0.5);
    z = -0.030 + progress * 0.035; // -0.030 to +0.005
    normX = -0.78;
    normY = 0.60;
    depth = 0.070;
...
async function testApparel(name, path, apparelKey, scaleUp = 1) {
  ...
  for (const step of steps) {
    const pl = getApparelSleevePlacement(apparelKey, "left_sleeve", 0, step.y);
